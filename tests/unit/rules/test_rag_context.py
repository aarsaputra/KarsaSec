...
def test_rag_predicate_allows_rule_only_when_rag_contains_text() -> None:
    ...
    metadata = RuleMetadataV2(
        name="RAG Rule", author="KarsaSec", version="1.0", enabled=True, tags=["use_rag", "rag_contains:ssrf"]
    )
    rule = Rule(
        id="KS-PY-RAG-0001",
        metadata=metadata,
        ...
        output=RuleOutput(
            severity=Severity.MEDIUM, confidence=Confidence.MEDIUM, message="RAG-based match", remediation="N/A"
        ),
    )
    ...
    rag_context = (
        {
            "document_id": "doc-1",
            "score": 0.92,
            "source_path": "owasp/cheatsheet.md",
            "text": "server-side ssrf example",
        },
    )

    scan_ctx = ScanContext(
        file_node=root, source_bytes=source, language="python", file_path=Path("main.py"), rag_context=rag_context
    )
