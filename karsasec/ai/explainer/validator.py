...
class EvidenceReferenceValidator:
    ...
    def validate(cls, explanation: SecurityExplanation, context: SecurityFindingContext) -> ValidationResult:
        ...
        if not has_sa_evidence:
            ...
            if cls._SANITIZER_KEYWORDS.search(sa_text) and not any(
                neg in sa_text.lower() for neg in ["none", "no ", "not ", "absence", "missing", "without"]
            ):
                errors.append(
        ...
        for claim in explanation.evidence_claims:
            if claim.claim_type == "SANITIZER" and claim.is_supported and not has_sa_evidence:
                errors.append(
                    f"AI claim '{claim.described_entity}' marked supported, but SAST evidence contains no sanitizer."
                )
...
class VerdictConsistencyValidator:
    ...
    _SAFE_CONTRADICTIONS = re.compile(
        r"\b(is safe|not vulnerable|false positive|no vulnerability|harmless|secure)\b", re.IGNORECASE
    )
...
class SecurityExplanationValidatorPipeline:
    ...
    def validate_and_sanitize(
        ...
        if not (context.sanitizer_evidence or context.sanitizer_constraints):
            corrected.sanitizer_analysis = (
                "NONE COMPATIBLE — Deterministic analysis confirmed no compatible sanitizer on flow path."
            )

        if context.verdict_status == "VULNERABLE":
            corrected.summary = f"Confirmed Vulnerable: {context.rule_title} ({context.cwe_id})"
            corrected.why_vulnerable = (
                f"Taint reaches sink without proven compatibility in {context.file_path}:{context.line_number}."
            )
